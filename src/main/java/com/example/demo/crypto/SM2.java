...
public class SM2 {
	...
	public static void setCurve(ECCurve.Fp curve) {
		SM2.curve = curve;
	}
...
//			byte buffer[] = publicKey.getEncoded(false);
			...
			FileUtils.writeStringToFile(file,encode,"utf-8");
//			fos.write(encode);
			fos.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void exportPrivateKey(BigInteger privateKey, String path) {
		...
		try {
			...
			String ecode=Base64.encodeBase64String(bytes);
			FileUtils.writeStringToFile(file,ecode,"UTF-8");
//			oos.writeObject(privateKey);
	...
	public BigInteger importPrivateKey(String path) {
		...
		try {
			...
			BigInteger res = Base64.decodeInteger(bytes);
			return res;
		} catch (Exception e) {
			e.printStackTrace();
		}
